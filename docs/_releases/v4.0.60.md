...
[HTTP Session](?id=Sessions)
model where after successful authentication a 
[Typed Users Session](?id=Sessions#using-typed-sessions-in-servicestack)
...
Like all of [ServiceStack's Auth Providers](?id=Authentication-and-authorization)
...
send them using ServiceStack's [.NET Service Clients](?id=CSharp-client):
...
[App Settings](?id=AppSettings) following the
...
[Multitenancy](?id=Multitenancy), the minimal configuration
...
As [mentioned in the last release](?id=2016-release-notes#deprecating-legacy-ormlite-apis)
...
added to [C#/.NET ServerEventsClient](?id=CSharp-Server-Events-Client)
