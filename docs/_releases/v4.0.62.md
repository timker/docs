...
ServiceStack's typed [C# Service Clients](?id=CSharp-client)
...
[encrypted JWT Cookie](?id=JWT-AuthProvider#encrypted-jwe-tokens)
...
after adding a [TypeScript ServiceStack Reference](?id=Add-ServiceStack-Reference)
...
[ServiceStack.Client](?id=CSharp-client) and
...
utilities simplifying many programming tasks which includes:
 ...
 [CSV](?id=CSV-Format) Text Serializers
 - [AutoMapping Utils](?id=Auto-mapping)
...
[C#/.NET Service Clients](?id=CSharp-client)
...
As [announced earlier this year](?id=v4-0-54#warning-net-40-builds-will-cease-after-august-1-2016)
...
## [JWT Auth Provider](?id=JWT-AuthProvider)
...
The same [HTTP Utils extension methods for Post and Put](?id=Http-Utils#posting-data)
...
### Disable Total in [AutoQuery](?id=AutoQuery-RDBMS)
