...
[.NET Core](https://www.microsoft.com/net/core#windows) which was boththe 
...
maximize knowledge and code-reuse and reducing portability efforts, all without breaking changes to existing 
.NET 4.5 Customers - realizing our primary goal.
...
This changes with .NET Core support which offers significant performance and stability improvements over Mono 
that's derived from a shared cross-platform code-base and supported by a well-resourced, active and responsive team. 
...
For .NET Core Live Demos we've settled on the popular power combo of:
 ...
 - Using [nginx-proxy](https://github.com/jwilder/nginx-proxy) settnig up an nginx reverse proxy and automatically bind virtual hosts to Docker Instances
...
Instance with a repeatable recipe letting you treat and deploy instances like opaque self-contained units.
...
We've extended the support of ServiceStack.Redis in the last release to now include 
OrmLite (SQL Server, Sqlite, PostgreSQL), ServiceStack, MVC Integration with Razor Pages, Swagger API, 
AutoQuery, AutoQuery Viewer, Stripe, AWS Integration with PocoDynamo and SQS, RedisMq, RabbitMq, ProtoBuf and Wire:
...
features that we're not able to support, these include:
 ...
 - **Markdown Razor** - CodeDom not supported in .NET Core (vanilla Markdown still supported)
...
behind a source-compatible `AppSelfHostBase` which can be used to create source-compatible SelfHost App Hosts.
...
all requests it receives, so it instead returns a `NotFoundHandler` and respond with a 404 Response.
...
providing source-compatible Reflection APIs like `Type.GetAssembly()` allowing you to configure source-compatible
...
The other change needed outside your ServiceStack implementation was to match .NET Core's default convention 
of serving static files from the **WebRoot** folder which just required moving all static resources into the 
...
developer ecosystem where .NET's **PascalCase** default protrudes in a sea of **camelCase** and 
...
[ss-utils normalize()](http://docs.servicestack.net/ss-utils-js.html#normalize-and-normalizekey) methods
can help with handling both conventions by recursively normalizing and converting all properties to **lowercase**.
...
If you need to resolve Request Scoped .NET Core dependencies you can resolve them using `IRequest`, e.g:
...
public override void Configure(Container container)
{
    services.RegisterAutoWirRegisterAutoWiredAsedAs<Scoped,IScoped>()
...
As ServiceStack can be hosted in variety of different platforms encompasesing several AppHost's, we've
...
```csharp
var file = appHost.MapProjectPath("~/path/to/settings.txt");
```
...
all RDBMS providers OrmLite queries using `UPPER(Field)` to ensure case-insensitive queries. 

However given .NET Core's strong focus on performance we're removing this feature and reverting to the 
...
Some other examples of HTTP Handlers you could use is returning an image by returning a `StaticFileHandler`
...
For this scenario we developed [Razor Pages that don't need either](http://razor.netcore.io/#smart-views), 
...
in both cases letting you access Request Params using **Model.Name** notation.

Razor Pages also lets you structure your Razor Views in whatever structure you want under `/wwwroot` 
which it will let you call using [Pretty URLs by default](http://razor.netcore.io/#no-ceremony) so you're 
not led into following the MVC-specific `{Controller}/{Action}` pattern or forced to define Custom Routes.
...
of live-reloading Razor Views behind-the-scenes so we're also able to get the same development experience 
as well. By using MVC Views we also naturally get good tooling support which 
...
over the JavaScript and CSS allows us to provide a more optimized browsing experience which we intend to 
...
Which [like other formats](http://docs.servicestack.net/csharp-client.html#httpwebrequest-service-clients) 
can be used using the `WireServiceClient` generic Typed Service Client, e.g:
...
It was also added to implement `IManageApiKeys` so it can now also be used as the persistent back-end
for the [API Key AuthProvider](http://docs.servicestack.net/api-key-authprovider.html).

Special thanks to [@ivanfioravanti](https://github.com/ivanfioravanti) for contributing the upgrade.

## [Add ServiceStack Reference](http://docs.servicestack.net/add-servicestack-reference.html)

[Kotlin Add ServiceStack Reference](http://docs.servicestack.net/kotlin-add-servicestack-reference.html)
was upgraded to support the latest [v1.04 release of Kotlin](https://blog.jetbrains.com/kotlin/2016/09/kotlin-1-0-4-is-here/).

### [TypeScript Add ServiceStack Reference](http://docs.servicestack.net/typescript-add-servicestack-reference.html)

TypeScript classes are now being generated in dependency order where dependent classes are generated first.
We've also removed `ExportAsTypes` as an inline config option. If you want to generate ambient Type declarations from the VS.NET Dialog
...
## Other ServiceStack Changes
 ...
 - The [JS Minifier](http://docs.servicestack.net/html-css-and-javascript-minification.html) was updated to the latest version by [@RobSchoenaker](https://github.com/RobSchoenaker)
 ...
 - `MultiAppSettings` now writes-thru to all [AppSettings providers](http://docs.servicestack.net/appsettings.html)
 ...
 - [Verb interface markers](http://docs.servicestack.net/csharp-client.html#http-verb-interface-markers) added to client Auth Request DTOs 
 - `HttpUtils.UseEncoding` can be used to override Text Encoding used in [HTTP Utils](http://docs.servicestack.net/http-utils.html)
...
OrmLite added support for deleting by querying from a joined table using a Typed SqlExpression, e.g:
