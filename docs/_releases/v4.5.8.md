...
ecosystem with new support for the Open API specification enabling new integration possibilities with Azure 
API Management and Microsoft's AutoRest, our real-time [Server Events](/server-events) solution extending to new 
Mobile, Desktop and Server platforms, more flexible Authentication options to dramatically simplify integration 
with Mobile, Desktop and Web clients, effortless JWT Refresh Tokens, enhanced support for TypeScript and 
Java client libraries, new compression options, upgrades to the latest Fluent Validation with support for 
Async validators, Async Request Filters, a number of new quality OSS ServiceStack projects being created 
by the community and lots more use-case driven features and refinements across the entire library suite!
...
The Open API Plugin is available in the new [ServiceStack.Api.OpenApi](https://www.nuget.org/packages/ServiceStack.Api.OpenApi)
NuGet package that can be installed with:
...
which lets you explore and call your Services from a dynamic UI generated from your Services Open API spec:
...
how your Services are documented in the Open API spec and resulting `/swagger-ui`, e.g: 
...
This is available in the Swagger UI via "Authorize" button.
...
Alternatively you can authenticate outside Swagger (e.g. via an OAuth Provider) and then return to 
`/swagger-ui` as an Authenticated User that's able to call protected Services.
...
With Open API support this is now possible where you can create a new Service in **API management services** in 
your Azure portal:
...
Then once Management API is activated (which can take >40mins), click on your API that was just created:

![](/images/azure-api-management/4-activating.png)

![](/images/azure-api-management/5-publisher-portal.png)

Then under **API** select **OpenAPI specification**:

![](/images/azure-api-management/6-add-api.png)

Which will let you either add the url to your Services `/openapi` endpoint or if preferred you can upload a 
local `openapi.json` copy:

![](/images/azure-api-management/7-create-api.png)

If the import was successful you'll be able to see a list of available operations for your services:

![](/images/azure-api-management/8-created.png)

### [AutoRest](https://github.com/Azure/autorest)

Another popular tool you can use with Open API is [Microsoft's AutoRest](https://github.com/Azure/autorest) 
which generates client libraries for accessing RESTful web services in a number of different languages
which now includes C#, Java, Node.js, TypeScript, Python and Ruby.
...
Or by using `iwr` if you have PowerShell installed:
...
## The Expanding Server Events Universe
...
standard enabling real-time server notifications over HTTP. Our solution enhances this simple lightweight protocol with a 
rich pub/sub system allowing for [flexible and targeted messaging](/server-events#sending-server-events)
by intelligent clients providing a variety of different strategies for easily handling events, built-in APIs to easily 
modify active subscriptions and querying the state of subscribed channels as well as built-in heartbeats and auto-retries 
for establishing hassle-free persistent connections to backend servers.

We've made great strides in our goal of expanding the reach of the Server Event ecosystem in popular Mobile, Desktop and 
Server platforms with new first-class implementations for Java and TypeScript added in this release which now includes:
...
Our C#, TypeScript and Java Server Event Clients are ports with full feature parity that offers the same functionality 
behind APIs that are idiomatic to their respective programming language. The Integration test suite has also been ported 
to each platform to assert behavior conformance and provides a good reference showing the aesthetics of using the Server 
Events Client in each language:
...
[Add ServiceStack Reference](/add-servicestack-reference) and ServiceStack's high-level Service Clients significantly
reduces the porting efforts for maintaining Native Client Apps in different platforms and languages whilst maximizing 
knowledge investments and reuse as every client works the same way and has the same features so knowledge and documentation 
is instantly transferable between each supported language.
...
On Android the `AndroidServerEventsClient` class should be used which enhances its Java `ServerEventsClient` sub class
with additional Async APIs and also contains platform-specific logic for handling behavioral differences in Android's
implementation of JVM's `HttpURLConnection`. 

Apps running on the JVM can use `ServerEventsClient` from the **net.servicestack:client** Java library. As their direct sub-classes the 2 classes are source-compatible for the most part with all APIs available in `ServerEventsClient` also available to `AndroidServerEventsClient`.

Just like C#, you can connect to Server Events with the **baseUrl** and **channels** you want to connect to.
The example below provides a good overview exploring how to configure and use different features of the 
`ServerEventsClient`: 
...
Full documentation exploring all `ServerEventsClient` features are available in the 
...
Since [Java Add ServiceStack Reference](/java-add-servicestack-reference) support and [Java Server Events Client](/java-server-events-client) are idiomatic ports of their C# equivalents, both projects utilized a message-based Typed API 
that significantly reduces the effort to port from their original C# sources, rendering the effort down to a 
...
#### [Android Java Chat Documentation](https://github.com/ServiceStackApps/AndroidJavaChat)

Full documentation walking through its implementation and explaining core functionality are available in the
[AndroidJavaChat Github project](https://github.com/ServiceStackApps/AndroidJavaChat). We'll explore some of its
key features below to give you a flavor of some of the capabilities available:
...
OAuth endpoint. After successful authentication the WebView is redirected back after having an Authenticated User Session
created on the server and [Session Cookies populated on the Response](/sessions). The Cookies are copied from the WebView 
before it's dismissed then used to populate the ServerEventsClient's Cookies so it's able to establish an Authenticated connection.

As Xamarin.Auth isn't available to Java on Android, we can instead take advantage of getting direct access to the latest 
3rd Party Android components and leverage Facebook's and Twitter's SDKs to handle the OAuth flow for us. We can then use 
the resulting User's AccessToken and utilize another new feature in this release (announced later) to Authenticate with 
a ServiceStack Server using just the Users OAuth AccessToken.

As they offer different level of customizations we've implemented 2 Login Activities, the 1st Activity shows how to 
integrate using Facebook's, Twitter's and Google's SDK Login buttons whilst the 2nd Activity shows how to use their SDK classes directly to be able to use your own preferred image login buttons. We'll go through using Facebook's Login 
button below but you can find all implementations for Facebook, Twitter and Google Sign In's
[explained in detail in the repo](https://github.com/ServiceStackApps/AndroidJavaChat#integrated-facebook-and-twitter-logins).

### LoginButtonsActivity

After [Installing Facebook's SDK](https://developers.facebook.com/docs/android/getting-started#androidstudio) and registering
your [ApplicationId in the AndroidManifest.xml](https://github.com/ServiceStackApps/AndroidJavaChat#signing-in-with-facebook-login-button) you can embed 
Facebook's Login Button widget in your Activity's [layout.xml](https://github.com/ServiceStackApps/AndroidJavaChat/blob/master/src/androidchat/app/src/main/res/layout/login_buttons.xml):
...
simply a matter of handling the Facebook `onSuccess()`, `onCancel()` and `onError()` callbacks. When the `onSuccess()` 
callback is fired it means the User has successfully Signed into our Android App, we then need to Authenticate with the 
...
it launched to capture the User's consent has completed:
...
Once the User has authenticated with the ServiceStack Server, the Authenticated Session Cookies are configured on the 
Service Client which can now be used to make authenticated requests.
...
After the User has successfully Authenticated with Facebook the first time you can access their saved token from Facebook's `AccessToken.getCurrentAccessToken()` to automatically sign them into ServiceStack Server on future restarts, e.g:
...
The TypeScript Server Event Client was also overhauled to match the behavior in C# and Java Server Events Client including its
...
The one additional feature the TypeScript (and JavaScript) Server Events Clients have over C# and Java is its built-in 
`css.*` handler which lets you automatically 
[modify the styles of any CSS Selector](/typescript-server-events-client#modifying-css) 
using the format:
...
All other non-Web Server Event clients will need to register a named CSS Selector to handle these `css.*` messages and 
translate its instructions to modify the styling in their own App as done in 
[CssReceiver.java](https://github.com/ServiceStackApps/AndroidJavaChat/blob/master/src/androidchat/app/src/main/java/servicestack/net/androidchat/CssReceiver.java) to modify the styling in Android Java Chat.

#### Breaking changes

As the TypeScript Server Events Client now follows the multi-platform C#/Java API and behavior you'll now need to explicitly 
call `.start()` to connect to the Server Events Stream and start listening for messages.

The type definitions for the built-in Server Event Messages have also been renamed to match their equivalent C#/Java Type names, e.g:
`ServerEventMessage`, `ServerEventConnect`, `ServerEventJoin`, etc.
...
runtime dependencies, essentially just [servicestack-client](https://github.com/ServiceStack/servicestack-client) and its 
es6-shim and W3C `EventSource` polyfills on Node.js and React Native where it's missing a native implementation.
...
has the same functionality as the Web App except instead of connecting to the [chat.servicestack.net](http://chat.servicestack.net) Events stream on the client, all connections are made in node.js and it's only the server state that's sent to the client which uses 
it to render the UI.

As the functionality of the app remains the same the Node.js App is able to reuse the existing `dtos.ts`, `.html` and .`css` from the 
Web App:
...
For the React Native App we've stuck with React Native default of using ES6/JavaScript instead of TypeScript but 
are still able to enjoy the same simplified programming model and concrete Types that 
...
Now the Server APIs can be consumed as normal by populating a Request DTO and sending it using a generic 
`JsonServiceClient`:
...
- [/types/typescript](http://techstacks.io/types/typescript) - Server Concrete Types
- [/types/typescript.d](http://techstacks.io/types/typescript.d) - Server Ambient Interface Definitions

These were previously both generated to enable an optimal end-to-end Typed API in the TypeScript **servicestack-client** 
which needs to embed metadata in Request DTOs to retain necessary type information that remains available after 
minification.

However a number of Customers instead preferred to continue using their own HTTP Client of choice (e.g. [restangular](https://github.com/2muchcoffeecom/ng2-restangular)) and would instead prefer to use the ambient interface definitions 
to add compiler Type Checking to their object literal requests, e.g:

```ts
let request:QueryContracts = { accountId: 1234 };
```

This use-case was hindered by the additional metadata requirements on Request DTOs which have now been relaxed in this 
release so they can be used to Type object literals as above. But as metadata in Request DTO interface definitions are 
no longer inferrable their return Type and route will need to be supplied on each call-site, e.g:
...
But if you are using the TypeScript `JsonServiceClient` we recommend instead using the Concrete Request DTOs to enable 
a succinct Typed API with minimal boilerplate, e.g:
...
populated `AuthenticateResponse` DTO and assign the Authenticated [Session Cookies](/sessions) to the Service
Client, enabling it to call protected Services on subsequent requests.

## JWT Refresh Tokens

Support for Refresh Tokens is now available in the [JWT Auth Provider](/jwt-authprovider) where new **JWT Tokens**
can now be accessed from a longer-lived `RefreshToken`. JWT is our 
[preferred choice for Microservices](/jwt-authprovider#stateless-auth-microservices)
as it enables Stateless Auth where only 1 Microservice needs to be configured to support and manage 
User Authentication and be able to issue JWT Tokens. This alleviates the configuration and management burden from 
all other Microservices as they'll be able to Authenticate Services by just validating and populating the 
User's Session embedded in JWT Tokens. JWT is also a high-performance solution that unlike every other 
Auth Provider requires no I/O or Server state to authenticate the user and populate its User Session.
...
aren't visible and won't take effect until the JWT Token expires and the User needs access to a new one. 
Previously this meant the User would need to Authenticate again to generate a new JWT Token which depending on 
the Auth Provider used would require resending Username/Password credentials, an API Key or initiate a new OAuth flow.

This can now be transparently handled with Refresh Tokens which can be used to generate new JWT Tokens without
...
These expiry times are very use-case specific so you'll want to check what values are appropriate for your System.
...
JWT Tokens using the same Refresh Token before needing to re-authenticate and get a new one.
...
including being sent as a **Bearer Token**, e.g:
...
the built-in `GetAccessToken` Service with your Refresh Token to generate a new Access Token, e.g:
...
Whilst this shows the sequence of events that lead to getting a new JWT Token, we can avoid the dance of handling
every Web Service Exception with the `OnAuthenticationRequired` callback which will get invoked anytime the 
Service Client receives a **401 Unauthorized** Response. We can use this callback to fetch a new JWT Token before 
auto-retrying the same request again, e.g:
...
In order to provide the simplest development experience possible we've gone a step further and made it so you
...
By default Service Clients will assume they should call the same ServiceStack Instance to fetch a new JWT Token.
If instead refresh tokens need to be sent to a different server, you'll need to specify the url it should use 
instead in the `RefreshTokenUri` property, e.g:
...
> The above `RefreshToken` capability is now available in all Service Clients

### Requires User Auth Repository

The one limitation for Refresh Tokens is that it must be configured to use a User Auth Repository which
is the persisted data source used to create the User Session that's embedded in the JWT Token.
...
Keep in mind a large appeal of JWT's are its performance and scalability characteristics which require no
I/O calls to Authenticate Users so it's preferable to apply custom logic when creating or refreshing JWT
tokens instead of each time the JWT Token is used. Shortening the expiry of JWT Tokens is another solution 
if custom validation logic is needed to be applied more frequently.

## Fluent Validation Upgraded

Another major feature added in this release was from the community with 
...
of the new implementation and existing Error Codes were preserved to ensure minimal disruption with existing
code bases.
...
Async validators can be registered using the `MustAsync` validator where you could simulate the built-in 
...
call an Async `GetStringLength` Service that returns the same `ErrorCode` and Error Message as the **Note Empty** 
validator with:
...
`validator.ValidateAsync()` if the code-path contains an Async Validator, e.g:
...
So if the `MyRequest` Service was called using the HTTP **POST** method it will call `validator.ValidateAsync()` 
...
To implement Async Validators properly we also needed Async Request Filters which were also added in this release 
...
> Note this is unnecessary when returning [cached responses](/http-caching) as ServiceStack automatically caches and 
returns the most optimal Response, which is typically compressed bytes for clients that support compression
...
is already compressing static files in which case you won't want to enable it in ServiceStack.

No compression is added when running ServiceStack in a self-host in which case you're free to configure ServiceStack 
...
This plugin is extremely versatile and supports persisting event subscriptions in 
[multiple back-end persistence stores](https://github.com/jezzsantos/ServiceStack.Webhooks/wiki/Plugins) 
including: Memory, Cache Client, Azure Table Storage as well as every major 
[RDBMS that OrmLite supports](https://github.com/ServiceStack/ServiceStack.OrmLite#download).
...
any `ICacheClient` to prefix all cache keys with the specified prefix, so you could for instance easily enable 
multi-tenant usage of a single redis instance with:
...
easier where it can be made to apply a custom `x.IsDeleted != true` condition on every `SqlExpression` using
a `SelectFilter` on concrete POCO Table Types, e.g:
...
Alternatively you can use a generic delegate that applies to all SqlExpressions, but you'll get access to a 
`IUntypedSqlExpression` instead of a `SqlExpression<T>` which offers a limited API surface area but will still
let you execute a custom filter for all `SqlExpression<T>` which you can use to add a condition for all tables
implementing a custom `ISoftDelete` interface, e.g:
...
You can easily escaping HTML Entities characters (`<`, `>`, `&`, `=`, `'`) when serializing JSON strings with:
...
you customize what options Rabbit MQ Queue's and topics get created with. So you could use this to 
[declare a queue with a custom TTL](https://www.rabbitmq.com/ttl.html) to have messages automatically expire 
...
individual Rogue plugin wont prevent your ServiceStack AppHost from starting. But when a plugins doesn't 
work properly it can be hard to determine the cause was due to an Exception occuring at registration. 
To better highlight the presence of Startup Errors we're now adding a red warning banner in `/metadata` 
pages in [DebugMode](/debugging#debugmode), e.g:
...
able to easily notice it when debugging HTTP Traffic.
...
want adhoc Services available on a particular format, for instance you may only want the View Models for your
dynamic Web Views available in HTML. You can now easily force this with the new `[HtmlOnly]` Request Filter Attribute, e.g:
...
This is also available for other built-in Content Types: `[JsonOnly]`, `[XmlOnly]`, `[JsvOnly]` and `[CsvOnly]`.
...
it will get called back with a populated [AuthFilterContext](https://github.com/ServiceStack/ServiceStack/blob/dfefd50b6ab5f03fce4f6dbbf445ec08150e0cba/src/ServiceStack/Auth/IAuthProvider.cs#L51) which can be used to modify properties on the `AuthenticateResponse` or if you want to completely
replace what's returned you can specify a `AuthFeature.AuthResponseDecorator`.

### ICustomUserAuth

The `ICustomUserAuth` interface can be implemented on User Auth Repositories that allow replacing the custom 
`UserAuth` and `UserAuthDetails` tables by returning the concrete Type that should be used instead, e.g:
...
This will allow using the same `RegistrationFeature` and `RegisterService` to handle registering new users
...
`IAuthSession.AuthProvider` which could be used for instance to detect and ensure highly sensitive services
are only available to users that authenticated using a specific Auth Provider.
...
a particular Windows Auth role however this can be a slow when needing to query a large number of roles
...
Docker Image since `microsoft/dotnet:latest` was changed to only be able to work with VS 2017 `.csproj` 
msbuild format.
